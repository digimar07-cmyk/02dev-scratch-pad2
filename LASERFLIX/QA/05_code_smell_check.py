...
def run_ruff(targets: list[str], s: Summary) -> str:
    ...
    lines += ["", f"  Total   : {bold(red(str(len(issues))))}", f"  Fixável : {green(str(fixable))}"]
    out = "\n".join(lines)
    print(out)
    return out
        ...
        for fp, blocks in (json.loads(out_cc) if out_cc.strip() else {}).items():
            for b in blocks:
                ...
                if cc >= CC_ALERT:
                    ...
                    lines.append(f"  {red('🔴 CRÍTICO')}  CC={bold(str(cc))} [{rank}]  {fp}:{ln} → {bold(name)}()")
                elif cc >= CC_WARN:
                    s.cc_warnings += 1
                    worst.append((fp, name, cc, ln, rank))
                    lines.append(f"  {yellow('🟡 ATENÇÃO')}  CC={bold(str(cc))} [{rank}]  {fp}:{ln} → {bold(name)}()")
    except (json.JSONDecodeError, TypeError):
        lines.append(dim("  (CC indisponível)"))
    s.worst_functions = sorted(worst, key=lambda x: -x[2])[:10]
    if not lines:
        lines.append(green("  ✅ Nenhuma função com CC alto"))
    ...
    try:
        ...
        if bad:
            for fp, v, r in sorted(bad, key=lambda x: x[1]):
                lines.append(f"  {red('⚠️')} MI={bold(f'{v:.1f}')} [{r}]  {fp}")
        else:
            lines.append(green(f"  ✅ Todos MI >= {MI_THRESHOLD}"))
    except (json.JSONDecodeError, TypeError):
        lines.append(dim("  (MI indisponível)"))
    out = "\n".join(lines)
    print(out)
    return out
...
def run_vulture(targets: list[str], s: Summary) -> str:
    ...
    for line in stdout.splitlines():
        ...
        items.append(line)
        low = line.lower()
        if "unused function" in low:   lines.append(f"  {red('🔴 função')}     {line}")
    ...
    if not lines:
        lines.append(green("  ✅ Nenhum dead code!"))
    else:
        lines.append(f"\n  Total: {bold(red(str(len(items))))} itens (confiança >= {VULTURE_CONF}%)")
    out = "\n".join(lines)
    print(out)
    return out
...
def calc_score(s: Summary) -> tuple[int, str, str]:
    ...
    if score >= 80: return score, "A", green(f"✅ SAUDÁVEL ({score}/100)")
    if score >= 60: return score, "B", yellow(f"⚠️  ATENÇÃO ({score}/100)")
    if score >= 40: return score, "C", yellow(f"🔶 PROBLEMÁTICO ({score}/100)")
...
def save_and_print_summary(s: Summary, sections: list[str]) -> None:
    ...
    print(bold("\n  💡 Próximos passos:"))
    if s.ruff_fixable > 0: print(f"    1. Execute 02_ruff_fix.bat ({s.ruff_fixable} auto-correções)")
    if s.cc_critical  > 0: print(f"    2. Refatorar {s.cc_critical} função(ões) CC ≥ 10")
    ...
    if score          >= 80:print(green("    ✅ Código saudável!"))
    # Salva
    ...
    print(f"\n  📄 Relatório: {bold(str(REPORT_FILE))}")
...
def main() -> None:
    ...
    if not existing:
        print(red("\n❌ Execute na raiz do projeto LASERFLIX!"))
        sys.exit(1)
